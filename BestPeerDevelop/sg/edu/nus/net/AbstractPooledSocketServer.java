...
public abstract class AbstractPooledSocketServer implements Runnable,
		PooledSocketServer {
	...
	public void acceptConnections() {
		try {
			...
			if (!stop) {
				JOptionPane.showMessageDialog(null, "Unable to bind port "
						+ port);
			}
			return;
		} catch (IOException e) {
			if (!stop) {
				JOptionPane.showMessageDialog(null,
						"I/O error happens at port: " + port);
			}
			return;
		}
	}
