...
public class SPUpdateRouteTableIndirectlyListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			SPUpdateRoutingTableIndirectlyBody body = (SPUpdateRoutingTableIndirectlyBody) msg
					.getBody();
			...
			if (!direction) {
				if (!special) {
					treeNode.getLeftRoutingTable().setRoutingTableNode(
							senderNodeInfo, index);
				} else {
					ChildNodeInfo newLeftChild = body.getInfoSender()
							.getLeftChild();

					RoutingItemInfo updateNodeInfo = (RoutingItemInfo) treeNode
							.getLeftRoutingTable().getRoutingTableNode(index);
			...
			if (((newNodeInfo.getLogicalInfo().getNumber() % 2) == 1)
			...
			if ((!direction) && (index == 0)
					&& ((newNodeInfo.getLogicalInfo().getNumber() % 2) == 0)
					&& (treeNode.getLeftChild() != null)) {
				tbody = new SPUpdateRoutingTableDirectlyBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						newNodeInfo, 0, direction, treeNode.getLeftChild()
								.getLogicalInfo());
			...
			if ((direction) && (index == 0)
					&& ((newNodeInfo.getLogicalInfo().getNumber() % 2) == 1)
					&& (treeNode.getRightChild() != null)) {
				tbody = new SPUpdateRoutingTableDirectlyBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						newNodeInfo, 0, direction, treeNode.getRightChild()
								.getLogicalInfo());
			...
			throw new EventHandleException(
					"Super peer indirectly updates routing table failure", e);
		}
	}

	public boolean isConsumed(Message msg) throws EventHandleException {
		if (msg.getHead().getMsgType() == MsgType.SP_UPDATE_ROUTING_TABLE_INDIRECTLY
				.getValue())
			return true;
		return false;
	}
