...
public class SPLeaveFindReplaceListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			SPLeaveFindReplacementNodeBody body = (SPLeaveFindReplacementNodeBody) msg
					.getBody();
			...
			if (treeNode.getLeftChild() != null) {
				...
				thead.setMsgType(MsgType.SP_LEAVE_FIND_REPLACEMENT_NODE
						.getValue());
				...
				i = 0;
				while ((i < leftRoutingTable.getTableSize())
						&& (transferInfo == null)) {
				...
				i = 0;
				while ((i < rightRoutingTable.getTableSize())
						&& (transferInfo == null)) {
				...
				if (transferInfo != null) {
					...
					if (transferInfo.getLeftChild() != null) {
						body.setLogicalDestination(transferInfo.getLeftChild()
								.getLogicalInfo());
						...
						body.setLogicalDestination(transferInfo.getRightChild()
								.getLogicalInfo());
			...
			throw new EventHandleException(
					"Super peer finds a replacement peer failure when it leaves network",
					e);
		}
	}

	private void doLeaveAndReplace(ServerPeer serverpeer, TreeNode treeNode,
			SPLeaveFindReplacementNodeBody body) {
		Message result = null;
		Head thead = new Head();
		...
		try {
			...
			adjacentInfo = new AdjacentNodeInfo(treeNode.getParentNode()
					.getPhysicalInfo(), treeNode.getParentNode()
					.getLogicalInfo());
			...
			if ((treeNode.getLogicalInfo().getNumber() % 2) == 0) {
				tbodyLeave = new SPLeaveBody(serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), treeNode.getContent(),
						treeNode.getRightAdjacentNode(), true,
						transferWorkList, treeNode.getParentNode()
								.getLogicalInfo());
				...
				tbodyLeave = new SPLeaveBody(serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), treeNode.getContent(),
						treeNode.getLeftAdjacentNode(), false,
						transferWorkList, treeNode.getParentNode()
								.getLogicalInfo());
			 ...
			 * ready to replace
			 */
			tbody = new SPLeaveFindReplacementNodeReplyBody(serverpeer
					.getPhysicalInfo(), body.getLogicalRequester());

			thead.setMsgType(MsgType.SP_LEAVE_FIND_REPLACEMENT_NODE_REPLY
					.getValue());
			result = new Message(thead, tbody);
	...
	public boolean isConsumed(Message msg) throws EventHandleException {
		if (msg.getHead().getMsgType() == MsgType.SP_LEAVE_FIND_REPLACEMENT_NODE
				.getValue())
			return true;
		return false;
	}
