	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if ((indexLeftRTMiss != -1) || (indexRightRTMiss != -1)) {
				...
				body.setLogicalDestination(treeNode.getParentNode()
						.getLogicalInfo());
				...
				// routing table is full, can insert new node or forward request
				...
				if ((treeNode.getLeftChild() == null)
					...
					if (treeNode.getLeftChild() == null) {
						...
						logicalNewNode = new LogicalInfo(childLevel,
								childNumber);

						childNode = new ChildNodeInfo(body.getNewNode(),
								logicalNewNode);
						treeNode.setLeftChild(childNode);

						numOfExpectedRTReply = PeerMath
								.getNumberOfExpectedRTReply(true, treeNode
										.getRightChild() != null, treeNode
						...
						numOfExpectedRTReply = PeerMath
								.getNumberOfExpectedRTReply(false, treeNode
										.getLeftChild() != null, treeNode
										.getLeftRoutingTable(), treeNode
										.getRightRoutingTable());
					}
					...
					if ((childNumber % 2) == 0) {
						...
						RoutingItemInfo newNodeInfo = SPGeneralAction.doAccept(
								serverpeer, treeNode, body.getNewNode(),
						...
						if (treeNode.getLeftAdjacentNode() == null) {
							content = SPGeneralAction.splitData(treeNode,
									false, serverpeer.getOrder());

							// update routing table for new node
							RoutingItemInfo newNodeInfo = SPGeneralAction
									.doAccept(serverpeer, treeNode, body
											.getNewNode(), new LogicalInfo(
											childLevel, childNumber), content,
											numOfExpectedRTReply, false, false);
							SPGeneralAction.updateRoutingTable(serverpeer,
									treeNode, newNodeInfo);
						} else {
							// the new node will share workload with the node's
							// left adjacent
							// ProcessGeneral.updateRangeValues(peerNode,
							// treeNode);
							RoutingItemInfo parentNodeInfo = new RoutingItemInfo(
									serverpeer.getPhysicalInfo(), treeNode
											.getLogicalInfo(), treeNode
											.getLeftChild(), treeNode
											.getRightChild(), treeNode
											.getContent().getMinValue(),
									treeNode.getContent().getMaxValue());

							tbody = new SPJoinSplitDataBody(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo(), body.getNewNode(),
									logicalNewNode, numOfExpectedRTReply,
									treeNode.getLeftRoutingTable(), treeNode
											.getRightRoutingTable(),
									parentNodeInfo, treeNode.getRightChild(),
									treeNode.getLeftAdjacentNode()
											.getLogicalInfo());

							thead.setMsgType(MsgType.SP_JOIN_SPLIT_DATA
									.getValue());
							...
							treeNode.setLeftAdjacentNode(new AdjacentNodeInfo(
									body.getNewNode(), logicalNewNode));
						}
					}

					((ServerGUI) gui).updatePane(treeNode);
				} else {
					// have full children, select an adjacent node to forward
					// request
					...
					if (!isForwardedRequest) {
						...
						if (treeNode.getLeftAdjacentNode().getLogicalInfo()
								.getLevel() <= treeNode.getRightAdjacentNode()
								.getLogicalInfo().getLevel()) {
			...
			e.printStackTrace();
			throw new EventHandleException("Super peer joins network failure",
					e);
		}
	}
