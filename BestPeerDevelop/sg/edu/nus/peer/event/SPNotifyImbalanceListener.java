...
public class SPNotifyImbalanceListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if (isMissingNodeChild(treeNode.getLogicalInfo(), missingNode)) {
				if (((missingNode.getNumber() % 2) == 0)
					...
					numOfExpectedRTReply = PeerMath.getNumberOfExpectedRTReply(
							false, treeNode.getLeftChild() != null, treeNode
									.getLeftRoutingTable(), treeNode
									.getRightRoutingTable());
					if (!direction) {
						// create a new node with its content
						nodeInfo = SPGeneralAction.doAccept(serverpeer,
								treeNode, serverpeer.getPhysicalInfo(),
								missingNode, treeNode.getContent(),
								numOfExpectedRTReply, false, false);

						treeNode.setRightChild(new ChildNodeInfo(serverpeer
								.getPhysicalInfo(), missingNode));
						treeNode.getContent().deleteAll();
						treeNode.getContent().setMaxValue(
								treeNode.getContent().getMinValue());

						SPGeneralAction.updateRoutingTable(serverpeer,
								treeNode, nodeInfo);
						this.pullNode(serverpeer, treeNode, true);
					} else {
						// create a new node with empty content
						ContentInfo emptyContent = new ContentInfo(treeNode
								.getContent().getMaxValue(), treeNode
								.getContent().getMaxValue(), serverpeer
								.getOrder(), new Vector<IndexValue>());

						nodeInfo = SPGeneralAction
								.doAccept(serverpeer, treeNode, treeNode
										.getRightAdjacentNode()
										.getPhysicalInfo(), missingNode,
										emptyContent, numOfExpectedRTReply,
										true, false);
						treeNode.setRightChild(new ChildNodeInfo(treeNode
								.getRightAdjacentNode().getPhysicalInfo(),
								missingNode));

						// will update later, no need
						// SPGeneralAction.updateRoutingTable(peerNode,
						// treeNode, nodeInfo);
					}
				} else if (((missingNode.getNumber() % 2) == 1)
					...
					if (!direction) {
						// create a new node with empty content
						ContentInfo emptyContent = new ContentInfo(treeNode
								.getContent().getMinValue(), treeNode
								.getContent().getMinValue(), serverpeer
								.getOrder(), new Vector<IndexValue>());

						nodeInfo = SPGeneralAction.doAccept(serverpeer,
								treeNode, treeNode.getLeftAdjacentNode()
										.getPhysicalInfo(), missingNode,
								emptyContent, numOfExpectedRTReply, true, true);

						treeNode.setLeftChild(new ChildNodeInfo(treeNode
								.getLeftAdjacentNode().getPhysicalInfo(),
								missingNode));

						// will update later, no need
						// ProcessGeneral.updateRoutingTable(peerNode, treeNode,
						// nodeInfo);
					} else {
						nodeInfo = SPGeneralAction.doAccept(serverpeer,
								treeNode, serverpeer.getPhysicalInfo(),
						...
						treeNode.setLeftChild(new ChildNodeInfo(serverpeer
								.getPhysicalInfo(), missingNode));
						treeNode.getContent().deleteAll();
						treeNode.getContent().setMinValue(
				...
				for (int i = 0; i < leftRoutingTable.getTableSize(); i++) {
					...
					if (nodeInfo != null) {
						if (isMissingNodeChild(nodeInfo.getLogicalInfo(),
								missingNode)) {
							body
									.setPhysicalSender(serverpeer
											.getPhysicalInfo());
							body.setLogicalSender(treeNode.getLogicalInfo());
							body.setLogicalDestination(nodeInfo
									.getLogicalInfo());
			...
			throw new EventHandleException(
					"Super peer notifies imbalance status to other peers failure",
					e);
		}
	}
	 ...
	 * @param direction
	 */
	private void pullNode(ServerPeer serverpeer, TreeNode treeNode,
			boolean direction) throws EventHandleException {
		Message result = null;
		Head head = new Head();
		Body body = null;
		...
		try {
			if (serverpeer.getActivateStablePosition() != null) {
				TreeNode activateTreeNode = serverpeer
						.getActivateStablePosition().getTreeNode();
				if (treeNode.getLogicalInfo().equals(
						activateTreeNode.getLogicalInfo())) {
			...
			if (!direction) {
				body = new SPLBRotationPullBody(serverpeer.getPhysicalInfo(),
	...
	public boolean isConsumed(Message msg) throws EventHandleException {
		if (msg.getHead().getMsgType() == MsgType.SP_NOTIFY_IMBALANCE
				.getValue())
			return true;
		return false;
	}
