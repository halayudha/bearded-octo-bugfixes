	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			for (int i = 0; i < body.getData().size(); i++) {
				treeNode.getContent().insertData(
						(IndexValue) body.getData().get(i), 0);
			}
			...
			if (treeNode.getContent().getMaxValue().compareTo(
				...
				if (body.getMinValue().compareTo(
						treeNode.getContent().getMinValue()) < 0) {
					treeNode.getContent().setMinValue(body.getMinValue());
				}
				if (body.getMaxValue().compareTo(
			 ...
			 * (!treeNode.isProcessLoadBalance())) { if (!body.getDirection()) {
			 * SPGeneralAction.doLoadBalance(serverpeer, treeNode, false, true);
			 * } else { SPGeneralAction.doLoadBalance(serverpeer, treeNode,
			 * true, false); } }
			 */
		} catch (Exception e) {
			e.printStackTrace();
			throw new EventHandleException(
					"SP_LB_SPLIT_DATA operation failure", e);
		}
	}
