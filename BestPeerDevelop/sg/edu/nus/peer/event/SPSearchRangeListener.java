...
import sg.edu.nus.protocol.body.SPSearchRangeBody;
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			for (int i = 0; i <= storedData.size() - 1; i++) {
				...
				if ((searchedMinValue.compareTo(temp) <= 0)
						&& (temp.compareTo(searchedMaxValue) <= 0)) {
					searchResult.add(storedData.get(i));
				}
			}
			if (searchResult.size() != 0) {
				resultBody = new SPSearchRangeResultBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						searchResult, body.getLogicalRequester());
				...
				if (searchResult.size() != 0) {
					...
					while ((index >= 0) && (found == -1)) {
						...
						if (treeNode.getLeftRoutingTable().getRoutingTableNode(
								index) != null) {
							if (nodeInfo.getMaxValue().compareTo(
									searchedMinValue) > 0) {
								found = index;
							}
						}
					...
					if (found != -1) {
						...
						if (treeNode.getLeftChild() != null) {
							...
							thead
									.setMsgType(MsgType.SP_SEARCH_RANGE
											.getValue());
							result = new Message(thead, body);
							...
							if (treeNode.getLeftAdjacentNode() != null) {
								...
								thead.setMsgType(MsgType.SP_SEARCH_RANGE
										.getValue());
								result = new Message(thead, body);
				...
				if (searchResult.size() != 0) {
					...
					while ((index >= 0) && (found == -1)) {
						if (treeNode.getRightRoutingTable()
							...
							if (nodeInfo.getMinValue().compareTo(
									searchedMaxValue) <= 0) {
								found = index;
							}
						}
			...
			throw new EventHandleException(
					"Super peer performs range search failure", e);
		}
	}
