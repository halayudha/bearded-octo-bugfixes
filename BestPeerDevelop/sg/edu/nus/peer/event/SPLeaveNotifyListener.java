	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			case 3: // departure is the left neighbor node
				nodeInfo = treeNode.getLeftRoutingTable().getRoutingTableNode(
						index);
				if (nodeInfo != null)
					nodeInfo.setPhysicalInfo(physicalReplacer);
				break;
			case 4: // departure is the right neighbor node
				nodeInfo = treeNode.getRightRoutingTable().getRoutingTableNode(
						index);
				if (nodeInfo != null)
					nodeInfo.setPhysicalInfo(physicalReplacer);
				break;
			...
			throw new EventHandleException(
					"Super peer notifies its neighbors failure when it leaves network",
					e);
		}
	}
