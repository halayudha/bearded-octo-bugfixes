...
public class SPLIRoutingTableListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if (logicalFailedNode.getLevel() == treeNode.getLogicalInfo()
					.getLevel()) {
				...
				if (direction == SPLIRoutingTableBody.FROM_LEFT_TO_RIGHT) {
					updateInfo = treeNode.getLeftRoutingTable()
							.getRoutingTableNode(index);
				} else {
					updateInfo = treeNode.getRightRoutingTable()
							.getRoutingTableNode(index);
				}
				...
				Body tbody = new SPLIRoutingTableReplyBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						nodeInfo, index, !direction, updateInfo.getMinValue(),
						updateInfo.getMaxValue(), body.getLogicalRequester());
				Head thead = new Head();
				...
				if (index == 0) {
					if ((logicalFailedNode.getNumber() % 2 == 0)
						...
						RoutingItemInfo routingItem = treeNode
								.getRightRoutingTable().getRoutingTableNode(0);
						Body tbody2 = new SPLIChildReplyBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								routingItem.getLeftChild(), routingItem
										.getRightChild(), body
										.getLogicalRequester());
						...
						if (routingItem.getLeftChild() != null) {
							Body tbody3 = new SPLIUpdateParentBody(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo(), body
									.getPhysicalRequester(), routingItem
									.getLeftChild().getLogicalInfo());
							Head thead3 = new Head();
							thead3.setMsgType(MsgType.SP_LI_UPDATE_PARENT
									.getValue());
							Message result3 = new Message(thead3, tbody3);
							serverpeer.sendMessage(routingItem.getLeftChild()
									.getPhysicalInfo(), result3);
						}
						...
						if (routingItem.getRightChild() != null) {
							...
							thead3.setMsgType(MsgType.SP_LI_UPDATE_PARENT
									.getValue());
							Message result3 = new Message(thead3, tbody3);
							serverpeer.sendMessage(routingItem.getRightChild()
									.getPhysicalInfo(), result3);
						}
					}
					...
					if ((logicalFailedNode.getNumber() % 2 == 1)
						...
						RoutingItemInfo routingItem = treeNode
								.getLeftRoutingTable().getRoutingTableNode(0);
						Body tbody2 = new SPLIChildReplyBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								routingItem.getLeftChild(), routingItem
										.getRightChild(), body
				...
				if (logicalFailedNode.getNumber() % 2 == 1) {
					if (treeNode.getLeftChild() != null) {
						...
						Head thead = new Head();
						thead
								.setMsgType(MsgType.SP_LI_ROUTING_TABLE
										.getValue());
	...
	public boolean isConsumed(Message msg) throws EventHandleException {
		if (msg.getHead().getMsgType() == MsgType.SP_LI_ROUTING_TABLE
				.getValue())
			return true;
		return false;
	}
