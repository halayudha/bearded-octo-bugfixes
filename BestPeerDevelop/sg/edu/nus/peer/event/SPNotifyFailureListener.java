...
public class SPNotifyFailureListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if (isFailedNodeChild(treeNode.getLogicalInfo(), body
					.getFailedNode())) {
				// double check again, if it is failed, call
				// SPGeneralAction.startRecoveryProcess
			} else {
				// find a link leading to the parent of the failed node
				if (treeNode.getLogicalInfo().getLevel() <= body
						.getFailedNode().getLevel()) {
					...
					if (PeerMath.compareNodePosition(treeNode.getLogicalInfo()
							.getLevel(), treeNode.getLogicalInfo().getNumber(),
							body.getFailedNode().getLevel(), body
									.getFailedNode().getNumber())) {
						// this node is on the left of the failed node
						for (int i = treeNode.getRightRoutingTable()
								.getTableSize() - 1; i >= 0; i--) {
							RoutingItemInfo nodeInfo = treeNode
									.getRightRoutingTable()
									.getRoutingTableNode(i);
							if (nodeInfo != null) {
								if (isFailedNodeChild(
										nodeInfo.getLogicalInfo(), body
												.getFailedNode())) {
									...
									head.setMsgType(MsgType.SP_NOTIFY_FAILURE
											.getValue());
									result = new Message(head, body);
									serverpeer.sendMessage(nodeInfo
											.getPhysicalInfo(), result);
									break;
								}
							}
						...
						for (int i = treeNode.getLeftRoutingTable()
								.getTableSize() - 1; i >= 0; i--) {
							RoutingItemInfo nodeInfo = treeNode
