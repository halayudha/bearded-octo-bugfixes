...
import sg.edu.nus.protocol.body.SPLBFindLightlyNodeBody;
...
public class SPLBFindLightlyNodeListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			SPLBFindLightlyNodeBody body = (SPLBFindLightlyNodeBody) msg
					.getBody();
			...
			if (!direction) {
				if (!PeerMath.checkNodesSameTree(body.getLogicalOverloaded()
						.getLevel(), body.getLogicalOverloaded().getNumber(),
						treeNode.getLogicalInfo().getLevel(), treeNode
								.getLogicalInfo().getNumber())) {
					tbody = new SPLBNoRotationNodeBody(serverpeer
					...
					serverpeer
							.sendMessage(body.getPhysicalOverloaded(), result);
				} else {
					if ((treeNode.getRightChild() != null)
							&& (!treeNode.getRightChild().getLogicalInfo()
									.equals(body.getLogicalSender()))) {
						...
						if (((treeNode.getContent().getOrder() * 2) <= body
							...
							if (treeNode.getRightRoutingTable().getTableSize() > nextIndex) {
								temptInfo = treeNode.getRightRoutingTable()
										.getRoutingTableNode(nextIndex);
								if (temptInfo != null) {
									body.setPhysicalSender(serverpeer
											.getPhysicalInfo());
									body.setLogicalSender(treeNode
											.getLogicalInfo());
									body.setNextIndex(nextIndex + 1);
									body.setLogicalDestination(temptInfo
											.getLogicalInfo());

									thead
											.setMsgType(MsgType.SP_LB_FIND_LIGHTLY_NODE
													.getValue());
									result = new Message(thead, body);
									serverpeer.sendMessage(temptInfo
											.getPhysicalInfo(), result);
								} else {
									if ((nextIndex - 1) < 0) {
										tbody = new SPLBNoRotationNodeBody(
												serverpeer.getPhysicalInfo(),
												treeNode.getLogicalInfo(), body
														.getLogicalOverloaded());

										thead
												.setMsgType(MsgType.SP_LB_NO_ROTATION_NODE
														.getValue());
										result = new Message(thead, tbody);
										serverpeer.sendMessage(body
								...
								thead.setMsgType(MsgType.SP_LB_NO_ROTATION_NODE
										.getValue());
								result = new Message(thead, tbody);
								serverpeer.sendMessage(body
										.getPhysicalOverloaded(), result);
							}
						}
					}
				}
			} else {
				if (!PeerMath.checkNodesSameTree(treeNode.getLogicalInfo()
						.getLevel(), treeNode.getLogicalInfo().getNumber(),
						body.getLogicalOverloaded().getLevel(), body
								.getLogicalOverloaded().getNumber())) {
					tbody = new SPLBNoRotationNodeBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(), body
							.getLogicalOverloaded());
					...
					if ((treeNode.getLeftChild() != null)
						...
						if (((treeNode.getContent().getOrder() * 2) <= body
							...
							if (treeNode.getLeftRoutingTable().getTableSize() > nextIndex) {
								temptInfo = treeNode.getLeftRoutingTable()
										.getRoutingTableNode(nextIndex);
								if (temptInfo != null) {
									body.setPhysicalSender(serverpeer
											.getPhysicalInfo());
			...
			throw new EventHandleException(
					"Find an underload super peer failure", e);
		}
	}
	 ...
	 */
	private void doLeaveAndJoin(ServerPeer serverpeer, TreeNode treeNode,
			SPLBFindLightlyNodeBody body) {
		...
		try {
			AdjacentNodeInfo adjacentInfo = new AdjacentNodeInfo(treeNode
					.getParentNode().getPhysicalInfo(), treeNode
					.getParentNode().getLogicalInfo());

			// get transfer worklist
			Vector<String> transferWorkList = SPGeneralAction
					.getTransferWorkList(treeNode, treeNode.getParentNode()
							.getLogicalInfo().toString());
			...
			if ((treeNode.getLogicalInfo().getNumber() % 2) == 0) {
				tbody = new SPLeaveBody(serverpeer.getPhysicalInfo(), treeNode
						.getLogicalInfo(), treeNode.getContent(), treeNode
						.getRightAdjacentNode(), true, transferWorkList,
						treeNode.getParentNode().getLogicalInfo());

				if (treeNode.getRightAdjacentNode() != null) {
					tbody = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							adjacentInfo, false, treeNode
									.getRightAdjacentNode().getLogicalInfo());

					thead
							.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK
									.getValue());
				...
				tbody = new SPLeaveBody(serverpeer.getPhysicalInfo(), treeNode
						.getLogicalInfo(), treeNode.getContent(), treeNode
						.getLeftAdjacentNode(), false, transferWorkList,
						treeNode.getParentNode().getLogicalInfo());

				if (treeNode.getLeftAdjacentNode() != null) {
					tbody = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							adjacentInfo, true, treeNode.getLeftAdjacentNode()
									.getLogicalInfo());
			...
			for (int i = 0; i < leftRoutingTable.getTableSize(); i++) {
				...
				if (temptInfo != null) {
					...
					message = new Message(thead, tbody);
					serverpeer
							.sendMessage(temptInfo.getPhysicalInfo(), message);
				}
			}
			...
			tbody = new SPJoinForcedBody(serverpeer.getPhysicalInfo(), !body
					.getDirection(), body.getLogicalOverloaded());
	...
	public boolean isConsumed(Message msg) throws EventHandleException {
		if (msg.getHead().getMsgType() == MsgType.SP_LB_FIND_LIGHTLY_NODE
				.getValue())
			return true;
		return false;
	}
