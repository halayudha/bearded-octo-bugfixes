...
public class SPInsertListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if ((insertedData.compareTo(minValue) >= 0)
					&& (insertedData.compareTo(maxValue) < 0)) {
				...
				if ((treeNode.getContent().isOverloaded())
					...
					// else
					// SPGeneralAction.doLoadBalance(serverpeer, treeNode,
					// true, true);
				}
					...
					if (found != -1) {
						...
						if (treeNode.getLeftChild() != null) {
							...
							if (treeNode.getLeftAdjacentNode() != null) {
								...
								treeNode.getContent().insertData(insertedData,
										1);
			...
			e.printStackTrace();
			throw new EventHandleException("Super peer inserts index failure",
					e);
		}
	}
	 ...
	 * 
	 * @param serverpeer
	 *            the handler of <code>ServerPeer</code>
	 * @param treeNode
	 *            information of the node in the tree structure
	 * @param direction
	 *            direction of searching
	 */
	private void doRotate(ServerPeer serverpeer, TreeNode treeNode,
			boolean direction) {
		...
		try {
			if (!direction) {
				...
				tempInfo = treeNode.getLeftRoutingTable()
						.getRoutingTableNode(0);
			}

			if (tempInfo != null) {
				body = new SPLBFindLightlyNodeBody(
						serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(),
						serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), treeNode.getContent()
								.getOrder(), 1, direction, treeNode
								.getNumOfQuery(), tempInfo.getLogicalInfo());
				...
				body = new SPLBFindLightlyNodeBody(
						...
						serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), treeNode.getContent()
								.getOrder(), 0, direction, treeNode
								.getNumOfQuery(), treeNode.getParentNode()
								.getLogicalInfo());
