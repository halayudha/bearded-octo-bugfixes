...
import sg.edu.nus.protocol.body.SPLIRoutingTableBody;
import sg.edu.nus.protocol.body.SPLIUpdateParentBody;
import sg.edu.nus.protocol.body.SPLeaveBody;
import sg.edu.nus.protocol.body.SPLeaveFindReplacementNodeBody;
...
public class SPGeneralAction {
	 ...
	 */
	public static void doLoadBalance(ServerPeer serverpeer, TreeNode treeNode,
			boolean canLeft, boolean canRight) {
		Message result = null;
		Head head = new Head();
		Body body = null;
		...
		try {
			...
			if ((canLeft) && (treeNode.getLeftAdjacentNode() != null)) {
				...
				body = new SPLBGetLoadInfoBody(serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), true, treeNode
			...
			if ((canRight) && (treeNode.getRightAdjacentNode() != null)) {
				...
				head.setMsgType(MsgType.SP_LB_GET_LOAD_INFO.getValue());
				body = new SPLBGetLoadInfoBody(serverpeer.getPhysicalInfo(),
						treeNode.getLogicalInfo(), false, treeNode
			...
			if ((treeNode.getLNElement() == -1)
					&& (treeNode.getRNElement() == -1)) {
				// can't split, should increase order
				treeNode.getContent().setOrder(
						treeNode.getContent().getOrder() * 2);
			} else {
		...
		try {
			...
			for (int j = 0; j < leftRoutingTable.getTableSize(); j++) {
				if (leftRoutingTable.getRoutingTableNode(j) != null) {
					...
					body = new SPUpdateMaxMinValueBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							nodeInfo, j, true, transferInfo.getLogicalInfo());
					result = new Message(head, body);
			...
			for (int j = 0; j < rightRoutingTable.getTableSize(); j++) {
				if (treeNode.getRightRoutingTable().getRoutingTableNode(j) != null) {
					...
					body = new SPUpdateMaxMinValueBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							nodeInfo, j, false, transferInfo.getLogicalInfo());
					result = new Message(head, body);
					serverpeer.sendMessage(transferInfo.getPhysicalInfo(),
							result);
				}
	...
	public static void updateRotateRoutingTable(ServerPeer serverpeer,
		...
		synchronized (treeNode) {
			try {
				...
				for (int j = 0; j < treeNode.getLeftRoutingTable()
						.getTableSize(); j++) {
					if (treeNode.getLeftRoutingTable().getRoutingTableNode(j) != null) {
						numOfExpectedRTReply++;
						transferInfo = treeNode.getLeftRoutingTable()
								.getRoutingTableNode(j);
						body = new SPLBRotateUpdateRoutingTableBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								nodeInfo, j, true, transferInfo
										.getLogicalInfo());

						head
								.setMsgType(MsgType.SP_LB_ROTATE_UPDATE_ROUTING_TABLE
										.getValue());
						result = new Message(head, body);
						serverpeer.sendMessage(transferInfo.getPhysicalInfo(),
								result);
				...
				for (int j = 0; j < treeNode.getRightRoutingTable()
						.getTableSize(); j++) {
					if (treeNode.getRightRoutingTable().getRoutingTableNode(j) != null) {
						numOfExpectedRTReply++;
						transferInfo = treeNode.getRightRoutingTable()
								.getRoutingTableNode(j);
						body = new SPLBRotateUpdateRoutingTableBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								nodeInfo, j, false, transferInfo
										.getLogicalInfo());
				...
				if (treeNode.getParentNode() != null) {
					if ((treeNode.getLogicalInfo().getNumber() % 2) == 0) {
						body = new SPLBRotateUpdateChildBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								true, treeNode.getParentNode().getLogicalInfo());
					} else {
						body = new SPLBRotateUpdateChildBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
					...
					head.setMsgType(MsgType.SP_LB_ROTATE_UPDATE_CHILD
							.getValue());
					result = new Message(head, body);
				...
				if (treeNode.getLeftChild() != null) {
					body = new SPLBRotateUpdateParentBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							treeNode.getLeftChild().getLogicalInfo());

					head.setMsgType(MsgType.SP_LB_ROTATE_UPDATE_PARENT
							.getValue());
					result = new Message(head, body);
				...
				if (treeNode.getLeftAdjacentNode() != null) {
					body = new SPLBRotateUpdateAdjacentBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							true, treeNode.getLeftAdjacentNode()
									.getLogicalInfo());

					head.setMsgType(MsgType.SP_LB_ROTATE_UPDATE_ADJACENT
							.getValue());
					result = new Message(head, body);
					serverpeer.sendMessage(treeNode.getLeftAdjacentNode()
							.getPhysicalInfo(), result);
				...
				if (treeNode.getRightAdjacentNode() != null) {
					body = new SPLBRotateUpdateAdjacentBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							false, treeNode.getRightAdjacentNode()
									.getLogicalInfo());
	...
	public static RoutingItemInfo doAccept(ServerPeer serverpeer,
		...
		try {
			if ((newNodeLogicalInfo.getNumber() % 2) == 0) {
				...
				newNodeLeftAdjacent = new AdjacentNodeInfo(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo());
				if (treeNode.getRightAdjacentNode() != null) {
					newNodeRightAdjacent = treeNode.getRightAdjacentNode();
					body = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							new AdjacentNodeInfo(newNodePhysicalInfo,
									newNodeLogicalInfo), false, treeNode
									.getRightAdjacentNode().getLogicalInfo());
				...
				treeNode.setRightAdjacentNode(new AdjacentNodeInfo(
						newNodePhysicalInfo, newNodeLogicalInfo));
			} else {
				newNodeRightAdjacent = new AdjacentNodeInfo(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo());
				if (treeNode.getLeftAdjacentNode() != null) {
					...
					body = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							new AdjacentNodeInfo(newNodePhysicalInfo,
									newNodeLogicalInfo), true, treeNode
									.getLeftAdjacentNode().getLogicalInfo());
				...
				treeNode.setLeftAdjacentNode(new AdjacentNodeInfo(
						newNodePhysicalInfo, newNodeLogicalInfo));
			}

			body = new SPJoinAcceptBody(serverpeer.getPhysicalInfo(), treeNode
					.getLogicalInfo(), newNodeLogicalInfo, newNodeLeftAdjacent,
					newNodeRightAdjacent, content, numOfExpectedRTReply,
					isFake, direction);
			...
			RoutingItemInfo newNodeInfo = new RoutingItemInfo(
					newNodePhysicalInfo, newNodeLogicalInfo, null, null,
					content.getMinValue(), content.getMaxValue());
	 ...
	 */
	public static ContentInfo splitData(TreeNode treeNode, boolean direction,
			int initialOrder) {
		...
		if (!direction) {
			...
			newNodeContentInfo = new ContentInfo(minValue, midValue, treeNode
					.getContent().getOrder(), transferData);
		} else {
			...
			newNodeContentInfo = new ContentInfo(midValue, maxValue, treeNode
					.getContent().getOrder(), transferData);
		}
	...
	public static void updateRoutingTable(ServerPeer serverpeer,
			TreeNode treeNode, RoutingItemInfo nodeInfo) {
		Message result = null;
		Head head = new Head();
		Body body = null;
		...
		try {
			senderInfo = new RoutingItemInfo(serverpeer.getPhysicalInfo(),
					treeNode.getLogicalInfo(), treeNode.getLeftChild(),
					treeNode.getRightChild(), treeNode.getContent()
							.getMinValue(), treeNode.getContent().getMaxValue());

			for (int i = 0; i < treeNode.getLeftRoutingTable().getTableSize(); i++) {
				if (treeNode.getLeftRoutingTable().getRoutingTableNode(i) != null) {
					destinationPhysicalInfo = treeNode.getLeftRoutingTable()
							.getRoutingTableNode(i).getPhysicalInfo();
					destinationLogicalInfo = treeNode.getLeftRoutingTable()
							.getRoutingTableNode(i).getLogicalInfo();

					body = new SPUpdateRoutingTableIndirectlyBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							senderInfo, nodeInfo, i, true, false,
							destinationLogicalInfo);

					head.setMsgType(MsgType.SP_UPDATE_ROUTING_TABLE_INDIRECTLY
							.getValue());
					result = new Message(head, body);
			...
			for (int i = 0; i < treeNode.getRightRoutingTable().getTableSize(); i++) {
				if (treeNode.getRightRoutingTable().getRoutingTableNode(i) != null) {
					destinationPhysicalInfo = treeNode.getRightRoutingTable()
							.getRoutingTableNode(i).getPhysicalInfo();
					;
					destinationLogicalInfo = treeNode.getRightRoutingTable()
							.getRoutingTableNode(i).getLogicalInfo();
					;
					body = new SPUpdateRoutingTableIndirectlyBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							senderInfo, nodeInfo, i, false, false,
							destinationLogicalInfo);
			...
			if (((nodeInfo.getLogicalInfo().getNumber() % 2) == 0)
					&& (treeNode.getLeftChild() != null)) {
				body = new SPUpdateRoutingTableDirectlyBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						nodeInfo, 0, true, treeNode.getLeftChild()
								.getLogicalInfo());

				head.setMsgType(MsgType.SP_UPDATE_ROUTING_TABLE_DIRECTLY
						.getValue());
				result = new Message(head, body);
				serverpeer.sendMessage(treeNode.getLeftChild()
						.getPhysicalInfo(), result);
			...
			if (((nodeInfo.getLogicalInfo().getNumber() % 2) == 1)
					&& (treeNode.getRightChild() != null)) {
				body = new SPUpdateRoutingTableDirectlyBody(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo(),
						nodeInfo, 0, false, treeNode.getRightChild()
								.getLogicalInfo());
	...
	public static void deleteTreeNode(ServerPeer serverpeer, TreeNode treeNode) {
		try {
			for (int i = 0; i < serverpeer.getListSize(); i++) {
				...
				if (temptNode.getLogicalInfo()
						.equals(treeNode.getLogicalInfo())) {
					// stop activate stable position process, should be only one
					// master node
					if (temptNode.getCoOwnerSize() > 0) {
						if (temptNode.getRole() == 1) {
							serverpeer.stopActivateStablePosition();
							Body body = new SPLBStablePositionBody(serverpeer
									.getPhysicalInfo(), temptNode
									.getLogicalInfo(), temptNode
									.getLogicalInfo());

							Head head = new Head();
							head.setMsgType(MsgType.SP_LB_STABLE_POSITION
									.getValue());
							Message result = new Message(head, body);
							for (int j = 0; j < temptNode.getCoOwnerSize(); j++) {
								serverpeer.sendMessage(temptNode
										.getCoOwnerList(j), result);
							}
						} else {
							// temptNode.role == 0
							for (int j = treeNode.getCoOwnerSize() - 1; j >= 0; j--) {
								if (treeNode.getCoOwnerList(j).equals(
	 ...
	 */
	public static Vector<String> getTransferWorkList(TreeNode treeNode,
		...
		for (int i = 0; i < treeNode.getWorkSize(); i++) {
			...
			if ((temp.indexOf("INSERT") != -1)
				...
				if (temp.substring(lastColon + 1).equals(
						treeNode.getLogicalInfo().toString())) {
	...
	public static boolean transferFakeNode(ServerPeer serverpeer,
			TreeNode treeNode, boolean direction, boolean isAccept,
		...
		try {
			...
			if (!direction) {
				for (i = 0; i < serverpeer.getListSize(); i++) {
					...
					if ((treeNode.getRightAdjacentNode().getLogicalInfo()
							.equals(temptNode.getLogicalInfo()))
							&& (!temptNode.getLeftAdjacentNode()
									.getLogicalInfo().equals(
											treeNode.getLogicalInfo()))) {
						// inconsist data
						if (isAccept) {
							AdjacentNodeInfo newAdjacent = new AdjacentNodeInfo(
									serverpeer.getPhysicalInfo(), treeNode
											.getLogicalInfo());
							temptNode.setLeftAdjacentNode(newAdjacent);
						} else {
							ActivateStablePosition activateStablePositionNode = serverpeer
									.getActivateStablePosition();
							if (activateStablePositionNode != null) {
								if (treeNode.getLogicalInfo()
										.equals(
												activateStablePositionNode
														.getTreeNode()
														.getLogicalInfo())) {
									serverpeer.stopActivateStablePosition();
								}
							...
							Body tbody = new SPLBRotationPullBody(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo(), treeNode, false);
							Head thead = new Head();
							thead.setMsgType(MsgType.SP_LB_ROTATION_PULL
									.getValue());
							...
							treeNode.addCoOwnerList(temptNode
									.getLeftAdjacentNode().getPhysicalInfo());
					...
					if (temptNode.getLeftAdjacentNode().getLogicalInfo()
							.equals(treeNode.getLogicalInfo())) {
						if (temptNode.getRole() == 0) {
							PhysicalInfo coOwner = (PhysicalInfo) temptNode
									.getCoOwnerList(0);
							treeNode.setRole(0);
							...
							if (SPGeneralAction.checkRotationPull(temptNode)) {
								...
								treeNode.setRightAdjacentNode(temptNode
										.getRightAdjacentNode());

								// 2. delete node
								deleteNode(serverpeer, treeNode, temptNode, i,
										false);
							} else {
								// need to update adjacent links
								AdjacentNodeInfo newRightAdjacent = new AdjacentNodeInfo(
										temptNode.getRightAdjacentNode()
												.getPhysicalInfo(), temptNode
												.getLogicalInfo());
								treeNode.setRightAdjacentNode(newRightAdjacent);
								if (isAccept) {
									RoutingItemInfo nodeInfo = new RoutingItemInfo(
											serverpeer.getPhysicalInfo(),
											treeNode.getLogicalInfo(), null,
											null, treeNode.getContent()
													.getMinValue(), treeNode
													.getContent().getMaxValue());

									Body tbody = new SPUpdateRoutingTableBody(
											serverpeer.getPhysicalInfo(),
											treeNode.getLogicalInfo(),
											nodeInfo, treeNode.getParentNode()
													.getLogicalInfo());
									Head thead = new Head();
									thead
											.setMsgType(MsgType.SP_UPDATE_ROUTING_TABLE
													.getValue());
									Message result = new Message(thead, tbody);
									serverpeer.sendMessage(treeNode
											.getParentNode().getPhysicalInfo(),
											result);
								}

								temptNode.setRole(0);
								AdjacentNodeInfo newLeftAdjacent = new AdjacentNodeInfo(
										serverpeer.getPhysicalInfo(), treeNode
												.getLogicalInfo());
								temptNode.setLeftAdjacentNode(newLeftAdjacent);

								Body tbody = new SPLBRotationPullBody(
										serverpeer.getPhysicalInfo(), temptNode
												.getLogicalInfo(), temptNode,
										false);
								Head thead = new Head();
								thead.setMsgType(MsgType.SP_LB_ROTATION_PULL
							...
							if (!isAccept) {
								SPGeneralAction.updateRotateRoutingTable(
										serverpeer, treeNode);
								return true;
							} else {
								return false;
				...
				if (activateStablePositionNode != null) {
					if (treeNode.getLogicalInfo().equals(
							activateStablePositionNode.getTreeNode()
									.getLogicalInfo()))
						serverpeer.stopActivateStablePosition();
				}
				...
				treeNode.clearCoOwnerList();
				treeNode.addCoOwnerList(treeNode.getRightAdjacentNode()
						.getPhysicalInfo());
				treeNode.deleteAllWork();
				...
				for (i = 0; i < serverpeer.getListSize(); i++) {
					...
					if ((treeNode.getLeftAdjacentNode().getLogicalInfo()
							.equals(temptNode.getLogicalInfo()))
							&& (!temptNode.getRightAdjacentNode()
									.getLogicalInfo().equals(
											treeNode.getLogicalInfo()))) {
					...
					if (temptNode.getRightAdjacentNode().getLogicalInfo()
							.equals(treeNode.getLogicalInfo())) {
						if (temptNode.getRole() == 0) {
							...
							treeNode.setContent(temptNode.getContent());
							if (SPGeneralAction.checkRotationPull(temptNode)) {
								// can delete tempt node without affecting the
								// tree balance
								// 1. must update adjacent link
								treeNode.setLeftAdjacentNode(temptNode
										.getLeftAdjacentNode());
								...
								AdjacentNodeInfo newRightAdjacent = new AdjacentNodeInfo(
										serverpeer.getPhysicalInfo(), treeNode
												.getLogicalInfo());
								temptNode
										.setRightAdjacentNode(newRightAdjacent);
				...
				treeNode.clearCoOwnerList();
				treeNode.addCoOwnerList(treeNode.getLeftAdjacentNode()
						.getPhysicalInfo());
				treeNode.deleteAllWork();
	...
	private static void deleteNode(ServerPeer serverpeer, TreeNode pullNode,
		...
		Vector transferWorkList = SPGeneralAction.getTransferWorkList(
				deleteNode, deleteNode.getParentNode().getLogicalInfo()
						.toString());

		try {
			if (!direction) {
				if (deleteNode.getLogicalInfo().getNumber() % 2 == 0) {
					// pullNode is deleteNode's parent, transfer worklist to
					// pullNode
					for (i = 0; i < transferWorkList.size(); i++)
						pullNode.putWork((String) transferWorkList.get(i));

					// 2. update adjacent links
					pullNode.setRightAdjacentNode(deleteNode
							.getRightAdjacentNode());
					if (deleteNode.getRightAdjacentNode() != null) {
						updateAdjacentInfoToAdjacent = new AdjacentNodeInfo(
								serverpeer.getPhysicalInfo(), pullNode
										.getLogicalInfo());

						Body tbody = new SPUpdateAdjacentLinkBody(serverpeer
								.getPhysicalInfo(), pullNode.getLogicalInfo(),
								updateAdjacentInfoToAdjacent, false, deleteNode
										.getRightAdjacentNode()
										.getLogicalInfo());
						Head thead = new Head();
						thead.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK
								.getValue());
						Message result = new Message(thead, tbody);
						serverpeer.sendMessage(deleteNode
								.getRightAdjacentNode().getPhysicalInfo(),
								result);
					}
					...
					// can be eliminated as there are lbrotateupdateroutingtable
					// msgs
					SPGeneralAction.updateRangeValues(serverpeer, pullNode);
				} else {
					// pullNode is deleteNode's left adjacent, delete node must
					// have a right
					...
					updateAdjacentInfoToParent = new AdjacentNodeInfo(
							serverpeer.getPhysicalInfo(), pullNode
									.getLogicalInfo());
					content = new ContentInfo(deleteNode.getContent()
							.getMaxValue(), deleteNode.getContent()
							.getMaxValue(), deleteNode.getContent().getOrder(),
							new Vector());

					Body tbody1 = new SPLeaveBody(serverpeer.getPhysicalInfo(),
							deleteNode.getLogicalInfo(), content,
							updateAdjacentInfoToParent, false,
							transferWorkList, deleteNode.getParentNode()
									.getLogicalInfo());
					Head thead1 = new Head();
					thead1.setMsgType(MsgType.SP_LEAVE.getValue());
					Message result1 = new Message(thead1, tbody1);
					serverpeer.sendMessage(deleteNode.getParentNode()
							.getPhysicalInfo(), result1);

					// 3. update adjacent link again
					updateAdjacentInfoToAdjacent = deleteNode
							.getRightAdjacentNode();

					Body tbody2 = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), deleteNode.getLogicalInfo(),
							updateAdjacentInfoToAdjacent, true, pullNode
									.getLogicalInfo());
					Head thead2 = new Head();
					thead1.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK
							.getValue());
					Message result2 = new Message(thead2, tbody2);
					serverpeer.sendMessage(serverpeer.getPhysicalInfo(),
							result2);
				}
			} else {
				...
				if (deleteNode.getLogicalInfo().getNumber() % 2 == 1) {
					...
					// 1. update adjacent links
					pullNode.setLeftAdjacentNode(deleteNode
							.getLeftAdjacentNode());
					if (deleteNode.getLeftAdjacentNode() != null) {
						...
						Body tbody = new SPUpdateAdjacentLinkBody(serverpeer
								.getPhysicalInfo(), pullNode.getLogicalInfo(),
								updateAdjacentInfoToAdjacent, true, deleteNode
										.getLeftAdjacentNode().getLogicalInfo());
						Head thead = new Head();
						thead.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK
								.getValue());
						Message result = new Message(thead, tbody);
						serverpeer.sendMessage(deleteNode.getLeftAdjacentNode()
								.getPhysicalInfo(), result);
					}
					...
					// pullNode is deleteNode's right adjacent, delete node must
					// have a left
					// adjacent node which is its parent
					...
					updateAdjacentInfoToParent = new AdjacentNodeInfo(
							serverpeer.getPhysicalInfo(), pullNode
									.getLogicalInfo());
					content = new ContentInfo(deleteNode.getContent()
							.getMinValue(), deleteNode.getContent()
							.getMinValue(), deleteNode.getContent().getOrder(),
							new Vector());
					...
					Body tbody2 = new SPUpdateAdjacentLinkBody(serverpeer
							.getPhysicalInfo(), deleteNode.getLogicalInfo(),
							updateAdjacentInfoToAdjacent, false, pullNode
									.getLogicalInfo());
					Head thead2 = new Head();
					thead1.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK
							.getValue());
			...
			for (i = 0; i < leftRT.getTableSize(); i++) {
				...
				if (temptInfo != null) {
					Body tbody = new SPUpdateRoutingTableDirectlyBody(
							serverpeer.getPhysicalInfo(), deleteNode
									.getLogicalInfo(), null, i, true, temptInfo
									.getLogicalInfo());
					Head thead = new Head();
			...
			// 5. stop activate stable position process, should be only one
			// master node
			// and delete node
			if (deleteNode.getCoOwnerSize() > 0) {
				serverpeer.stopActivateStablePosition();
				Body tbody = new SPLBStablePositionBody(serverpeer
						.getPhysicalInfo(), deleteNode.getLogicalInfo(),
						deleteNode.getLogicalInfo());
				Head thead = new Head();
				thead.setMsgType(MsgType.SP_LB_STABLE_POSITION.getValue());
				Message result = new Message(thead, tbody);
				for (int j = 0; j < deleteNode.getCoOwnerSize(); j++) {
					serverpeer.sendMessage((PhysicalInfo) deleteNode
							.getCoOwnerList(j), result);
				}
			}
	...
	public static void startRecoveryProcess(ServerPeer serverpeer,
			TreeNode treeNode, LogicalInfo failedNodeInfo) {
		Message result = null;
		Head head = new Head();
		Body body = null;
		...
		int leftRTSize = PeerMath.getRoutingTableSize(failedNodeNumber,
				failedNodeLevel, false);
		int rightRTSize = PeerMath.getRoutingTableSize(failedNodeNumber,
				failedNodeLevel, true);
		...
		ContentInfo content = new ContentInfo(treeNode.getContent()
				.getMinValue(), treeNode.getContent().getMaxValue(), treeNode
				.getContent().getOrder(), new Vector<IndexValue>());
		int numOfExpectedRTReply = 0;

		try {
			if (failedNodeLevel == 0) {
				// special case, the root is failed
				treeNode.getParentNode().setPhysicalInfo(
						serverpeer.getPhysicalInfo());
				...
				if (treeNode.getLogicalInfo().getNumber() % 2 == 0) {
					...
					ChildNodeInfo rightChild = new ChildNodeInfo(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo());
					rightAdjacentNode = new AdjacentNodeInfo(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo());
					content.setMinValue(new BoundaryValue(IndexValue.MIN_KEY
							.getString(), Long.MIN_VALUE));
					...
					ChildNodeInfo leftChild = new ChildNodeInfo(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo());
					ChildNodeInfo rightChild = null;

					RoutingItemInfo tempt = treeNode.getRightRoutingTable()
							.getRoutingTableNode(0);
					if (tempt == null) {
						treeNode.getRightAdjacentNode().setPhysicalInfo(
								serverpeer.getPhysicalInfo());

						content
								.setMaxValue(new BoundaryValue(
										IndexValue.MAX_KEY.getString(),
										Long.MAX_VALUE));
						TreeNode newTreeNode = new TreeNode(failedNodeInfo,
								null, leftChild, null, null, null,
								new RoutingTableInfo(0),
								new RoutingTableInfo(0), content, 0,
								TreeNode.ACTIVE);
						serverpeer.addListItem(newTreeNode);
						SPGeneralAction.doLeave(serverpeer, serverpeer
								.getPhysicalInfo(), treeNode);
					} else {
						rightChild = new ChildNodeInfo(tempt.getPhysicalInfo(),
						...
						if (treeNode.getRightChild() == null) {
							...
							leftAdjacentNode = new AdjacentNodeInfo(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo());
							content.setMinValue(treeNode.getContent()
									.getMaxValue());
						} else {
							destinationPhysicalInfo = treeNode.getRightChild()
									.getPhysicalInfo();
							;
							destinationLogicalInfo = treeNode.getRightChild()
									.getLogicalInfo();
							;

							body = new SPLIAdjacentRootBody(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo(), serverpeer
									.getPhysicalInfo(), failedNodeInfo,
									SPLIAdjacentRootBody.FROM_RIGHT_TO_LEFT,
									failedNodeInfo, destinationLogicalInfo);

							head.setMsgType(MsgType.SP_LI_ADJACENT_ROOT
									.getValue());
							result = new Message(head, body);
							serverpeer.sendMessage(destinationPhysicalInfo,
									result);
						}

						if (tempt.getLeftChild() == null) {
							rightAdjacentNode = new AdjacentNodeInfo(tempt
									.getPhysicalInfo(), tempt.getLogicalInfo());
							content.setMinValue(tempt.getMaxValue());
						} else {
							Body tbody1 = new SPLIUpdateParentBody(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo(), serverpeer
									.getPhysicalInfo(), tempt.getLogicalInfo());
							Head thead1 = new Head();
							thead1.setMsgType(MsgType.SP_LI_UPDATE_PARENT
									.getValue());
							Message result1 = new Message(thead1, tbody1);
							serverpeer.sendMessage(tempt.getPhysicalInfo(),
									result1);

							destinationPhysicalInfo = tempt.getLeftChild()
									.getPhysicalInfo();
							;
							destinationLogicalInfo = tempt.getLeftChild()
									.getLogicalInfo();
							;
							...
							body = new SPLIAdjacentRootBody(serverpeer
									...
									.getLogicalInfo(), serverpeer
									.getPhysicalInfo(), failedNodeInfo,
									SPLIAdjacentRootBody.FROM_LEFT_TO_RIGHT,
									failedNodeInfo, destinationLogicalInfo);
						...
						TreeNode newTreeNode = new TreeNode(failedNodeInfo,
								null, leftChild, rightChild, leftAdjacentNode,
								rightAdjacentNode, new RoutingTableInfo(0),
								new RoutingTableInfo(0), content, 0,
								TreeNode.ACTIVE);
						serverpeer.addListItem(newTreeNode);
						if ((leftAdjacentNode != null)
								&& (rightAdjacentNode != null)) {
							// this node must hold both the root and its left
							// child
				...
				for (int i = 0; i < treeNode.getLeftRoutingTable()
						.getTableSize(); i++) {
					if (treeNode.getLeftRoutingTable().getRoutingTableNode(i) != null) {
						numOfExpectedRTReply++;
						destinationPhysicalInfo = treeNode
								.getLeftRoutingTable().getRoutingTableNode(i)
								.getPhysicalInfo();
						destinationLogicalInfo = treeNode.getLeftRoutingTable()
								.getRoutingTableNode(i).getLogicalInfo();

						body = new SPLIRoutingTableBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								serverpeer.getPhysicalInfo(), failedNodeInfo,
								failedNodeInfo, i,
								SPLIRoutingTableBody.FROM_RIGHT_TO_LEFT,
								destinationLogicalInfo);
				...
				for (int i = 0; i < treeNode.getRightRoutingTable()
						.getTableSize(); i++) {
					if (treeNode.getRightRoutingTable().getRoutingTableNode(i) != null) {
						numOfExpectedRTReply++;
						destinationPhysicalInfo = treeNode
								.getRightRoutingTable().getRoutingTableNode(i)
								.getPhysicalInfo();
						;
						destinationLogicalInfo = treeNode
								.getRightRoutingTable().getRoutingTableNode(i)
								.getLogicalInfo();
						;
						...
						body = new SPLIRoutingTableBody(serverpeer
								...
								serverpeer.getPhysicalInfo(), failedNodeInfo,
								failedNodeInfo, i,
								SPLIRoutingTableBody.FROM_LEFT_TO_RIGHT,
								destinationLogicalInfo);
				...
				if (((failedNodeInfo.getNumber() % 2) == 0)) {
					...
					// update itself
					treeNode.getRightChild().setPhysicalInfo(
							serverpeer.getPhysicalInfo());

					if (treeNode.getLeftChild() != null) {
						numOfExpectedRTReply++;
						destinationPhysicalInfo = treeNode.getLeftChild()
								.getPhysicalInfo();
						;
						destinationLogicalInfo = treeNode.getLeftChild()
								.getLogicalInfo();
						;
						...
						if (treeNode.getRightAdjacentNode().getLogicalInfo()
								.equals(failedNodeInfo)) {
							// left adjacent node of the failed node is its
							// parent
							leftAdjacentNode = new AdjacentNodeInfo(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo());
							...
							if (treeNode.getRightAdjacentNode() != null) {
								...
								body = new SPLIAdjacentBody(serverpeer
										.getPhysicalInfo(), treeNode
										.getLogicalInfo(), serverpeer
										.getPhysicalInfo(), failedNodeInfo,
										SPLIAdjacentBody.FROM_RIGHT_TO_LEFT,
										failedNodeInfo, destinationLogicalInfo);

								head.setMsgType(MsgType.SP_LI_ADJACENT
										.getValue());
								result = new Message(head, body);
								serverpeer.sendMessage(destinationPhysicalInfo,
										result);
					...
					if (treeNode.getParentNode() != null) {
						numOfExpectedRTReply++;
						destinationPhysicalInfo = treeNode.getParentNode()
								.getPhysicalInfo();
						destinationLogicalInfo = treeNode.getParentNode()
								.getLogicalInfo();

						body = new SPLIAdjacentBody(serverpeer
								.getPhysicalInfo(), treeNode.getLogicalInfo(),
								serverpeer.getPhysicalInfo(), failedNodeInfo,
								SPLIAdjacentBody.FROM_LEFT_TO_RIGHT,
								failedNodeInfo, destinationLogicalInfo);
					...
					// update itself
					treeNode.getLeftChild().setPhysicalInfo(
							serverpeer.getPhysicalInfo());
					...
					if (treeNode.getRightChild() != null) {
						...
						if (treeNode.getLeftAdjacentNode().getLogicalInfo()
								.equals(failedNodeInfo)) {
							// right adjacent node of the failed node is its
							// parent
							rightAdjacentNode = new AdjacentNodeInfo(serverpeer
									.getPhysicalInfo(), treeNode
									.getLogicalInfo());
				...
				ParentNodeInfo parentNode = new ParentNodeInfo(serverpeer
						.getPhysicalInfo(), treeNode.getLogicalInfo());
				TreeNode newTreeNode = new TreeNode(failedNodeInfo, parentNode,
						null, null, leftAdjacentNode, rightAdjacentNode,
						leftRoutingTable, rightRoutingTable, content,
						numOfExpectedRTReply, TreeNode.FAILED);
				serverpeer.addListItem(newTreeNode);
				...
				if (numOfExpectedRTReply == 0) {
					...
					if (SPGeneralAction.checkRotationPull(newTreeNode)) {
						SPGeneralAction.doLeave(serverpeer, serverpeer
								.getPhysicalInfo(), newTreeNode);
					} else {
						SPGeneralAction.doFindReplacement(serverpeer,
								serverpeer.getPhysicalInfo(), newTreeNode);
					}
				} else {
					// ActivateFailedPosition activateFailedPosition =
					new ActivateFailedPosition(newTreeNode,
							ServerPeer.TIME_TO_RECOVER_FAILED_NODE);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void doLeave(ServerPeer serverpeer,
			PhysicalInfo physicalInfo, TreeNode treeNode) throws Exception {
		...
		adjacentInfo = new AdjacentNodeInfo(treeNode.getParentNode()
				.getPhysicalInfo(), treeNode.getParentNode().getLogicalInfo());

		transferWorkList = SPGeneralAction.getTransferWorkList(treeNode,
				treeNode.getParentNode().getLogicalInfo().toString());

		// 1. send update adjacent request to its adjacent node and leave
		// request to its parent
		if (treeNode.getLogicalInfo().getNumber() % 2 == 0) {
			leaveProtocol = new SPLeaveBody(physicalInfo, treeNode
					.getLogicalInfo(), treeNode.getContent(), treeNode
					.getRightAdjacentNode(), true, transferWorkList, treeNode
					.getParentNode().getLogicalInfo());
			if (treeNode.getRightAdjacentNode() != null) {
				updateAdjacentLinkProtocol = new SPUpdateAdjacentLinkBody(
						physicalInfo, treeNode.getLogicalInfo(), adjacentInfo,
						false, treeNode.getRightAdjacentNode().getLogicalInfo());
				Head head = new Head();
				head.setMsgType(MsgType.SP_UPDATE_ADJACENT_LINK.getValue());
				Message sendMessage = new Message(head,
						updateAdjacentLinkProtocol);
				serverpeer.sendMessage(treeNode.getRightAdjacentNode()
						.getPhysicalInfo(), sendMessage);
			}
		} else {
			leaveProtocol = new SPLeaveBody(physicalInfo, treeNode
					.getLogicalInfo(), treeNode.getContent(), treeNode
					.getLeftAdjacentNode(), false, transferWorkList, treeNode
					.getParentNode().getLogicalInfo());
			if (treeNode.getLeftAdjacentNode() != null) {
				updateAdjacentLinkProtocol = new SPUpdateAdjacentLinkBody(
						physicalInfo, treeNode.getLogicalInfo(), adjacentInfo,
		...
		serverpeer.sendMessage(treeNode.getParentNode().getPhysicalInfo(),
				sendMessage);

		// 2. notify its neighbor nodes
		for (i = 0; i < leftRoutingTable.getTableSize(); i++) {
			temptInfo = (RoutingItemInfo) leftRoutingTable
					.getRoutingTableNode(i);
			if (temptInfo != null) {
				updateRoutingTableDirectlyProtocol = new SPUpdateRoutingTableDirectlyBody(
						physicalInfo, treeNode.getLogicalInfo(), null, i, true,
						temptInfo.getLogicalInfo());
				Head head1 = new Head();
				head1.setMsgType(MsgType.SP_UPDATE_ROUTING_TABLE_DIRECTLY
						.getValue());
				Message sendMessage1 = new Message(head1,
						updateRoutingTableDirectlyProtocol);
				serverpeer.sendMessage(temptInfo.getPhysicalInfo(),
						sendMessage1);
			}
		}
		for (i = 0; i < rightRoutingTable.getTableSize(); i++) {
			temptInfo = (RoutingItemInfo) rightRoutingTable
					.getRoutingTableNode(i);
			if (temptInfo != null) {
				updateRoutingTableDirectlyProtocol = new SPUpdateRoutingTableDirectlyBody(
						physicalInfo, treeNode.getLogicalInfo(), null, i,
						false, temptInfo.getLogicalInfo());
				Head head1 = new Head();
				head1.setMsgType(MsgType.SP_UPDATE_ROUTING_TABLE_DIRECTLY
						.getValue());
				Message sendMessage1 = new Message(head1,
		...
		// 3. send stabilization request to slaves if it is master node and
		// there is any slave
		if (treeNode.getCoOwnerSize() > 0) {
			serverpeer.stopActivateStablePosition();
			SPLBStablePositionBody lbStablePositionProtocol = new SPLBStablePositionBody(
					physicalInfo, treeNode.getLogicalInfo(), treeNode
							.getLogicalInfo());
			Head head1 = new Head();
			...
			for (i = 0; i < treeNode.getCoOwnerSize(); i++) {
				serverpeer.sendMessage((PhysicalInfo) treeNode
						.getCoOwnerList(i), sendMessage1);
			}
		}
	...
	public static void doFindReplacement(ServerPeer serverpeer,
		...
		if ((treeNode.getLeftChild() != null)
			...
			if ((treeNode.getLeftAdjacentNode() != null)
				...
				if (treeNode.getLeftAdjacentNode().getLogicalInfo().getLevel() >= treeNode
						.getRightAdjacentNode().getLogicalInfo().getLevel()) {
					body = new SPLeaveFindReplacementNodeBody(physicalInfo,
							treeNode.getLogicalInfo(), physicalInfo, treeNode
									.getLogicalInfo(), treeNode
									.getLeftAdjacentNode().getLogicalInfo());
			...
			while ((i < treeNode.getLeftRoutingTable().getTableSize())
					&& (transferInfo == null)) {
				temptInfo = treeNode.getLeftRoutingTable().getRoutingTableNode(
						i);
				if (temptInfo != null)
					if ((temptInfo.getLeftChild() != null)
							|| (temptInfo.getRightChild() != null))
			...
			while ((i < treeNode.getRightRoutingTable().getTableSize())
					&& (transferInfo == null)) {
				temptInfo = treeNode.getRightRoutingTable()
						.getRoutingTableNode(i);
				if (temptInfo != null)
					if ((temptInfo.getLeftChild() != null)
							|| (temptInfo.getRightChild() != null))
			...
			if (transferInfo != null) {
				if (transferInfo.getLeftChild() != null) {
					body = new SPLeaveFindReplacementNodeBody(physicalInfo,
							treeNode.getLogicalInfo(), physicalInfo, treeNode
									.getLogicalInfo(), transferInfo
									.getLeftChild().getLogicalInfo());
					Message message = new Message(head, body);
					serverpeer.sendMessage(transferInfo.getLeftChild()
							.getPhysicalInfo(), message);
				} else {
					body = new SPLeaveFindReplacementNodeBody(physicalInfo,
							treeNode.getLogicalInfo(), physicalInfo, treeNode
					...
					Message message = new Message(head, body);
					serverpeer.sendMessage(transferInfo.getRightChild()
							.getPhysicalInfo(), message);
				}
			}
		}
	...
	public static Vector<IndexValue> loadData() {
		...
		try {
			...
			while (rs.next()) {
				...
				IndexValue indexValue = new IndexValue(IndexValue.TABLE_INDEX,
						ind, new IndexInfo(val));
				data.add(indexValue);
			}
			rs.close();
			...
			while (rs.next()) {
				String ind = rs.getString("ind");
				String val = rs.getString("val");
				IndexValue indexValue = new IndexValue(IndexValue.COLUMN_INDEX,
						ind, new IndexInfo(val));
				data.add(indexValue);
			}
			rs.close();
			...
			while (rs.next()) {
				String ind = rs.getString("ind");
				String val = rs.getString("val");
				IndexValue indexValue = new IndexValue(IndexValue.DATA_INDEX,
						ind, new IndexInfo(val));
				data.add(indexValue);
			}
			rs.close();
	...
	public static void deleteAllData() {
		try {
			...
			System.out
					.println("Error while deleting all data from bestpeerindexdb");
			e.printStackTrace();
		}
	}
		...
		try {
			...
			for (int i = 0; i < data.size(); i++) {
				...
				case IndexValue.TABLE_INDEX:
					String sql = "insert into table_index(ind, val) values ('"
							+ ind + "','" + val + "')";
					try{
						stmt.execute(sql);
					}
				...
				case IndexValue.COLUMN_INDEX:
					sql = "insert into column_index(ind, val) values ('" + ind
							+ "','" + val + "')";
					try{
						stmt.execute(sql);
					}
				...
				case IndexValue.DATA_INDEX:
					sql = "insert into data_index(ind, val) values ('" + ind
							+ "','" + val + "')";
					try{
						stmt.execute(sql);
					}
