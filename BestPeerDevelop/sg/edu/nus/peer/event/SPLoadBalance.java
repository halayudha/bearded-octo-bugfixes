...
public class SPLoadBalance {
	...
	public void balanceLoad() throws EventHandleException {
		...
		try {
			...
			} else if ((treeNode.getLNElement() * 2) > treeNode.getContent()
					.getData().size()) {
				treeNode.setLNElement(-1);
			}
			...
			} else if ((treeNode.getRNElement() * 2) > treeNode.getContent()
					.getData().size()) {
				treeNode.setRNElement(-1);
			}
			...
			if ((treeNode.getLNElement() == -1)
				...
				if ((newNumOfLNElement != -1)
						&& (newNumOfLNElement > treeNode.getLNElement())) {
					...
					for (i = numOfData - 1; i >= 0; i--) {
						transferData
								.add(treeNode.getContent().getData().get(i));
						treeNode.getContent().getData().remove(i);
					}

					BoundaryValue newNodeMinValue;
					IndexValue minValue = (IndexValue) treeNode.getContent()
							.getData().get(0);
					if (minValue.getType() == IndexValue.NUMERIC_TYPE)
						newNodeMinValue = new BoundaryValue(oldMinValue
								.getStringValue(), Long.parseLong(minValue
								.getString()));
					else
						newNodeMinValue = new BoundaryValue(minValue
								.getString(), oldMinValue.getLongValue());

					treeNode.getContent().setMinValue(newNodeMinValue);

					Body body = new SPLBSplitDataBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							true, oldMinValue, treeNode.getContent()
									.getMinValue(), transferData, treeNode
				...
				if ((newNumOfRNElement != -1)
						&& (newNumOfRNElement > treeNode.getRNElement())) {
					// split data to its right adjacent node
					int numOfData = (newNumOfRNElement - treeNode
							.getRNElement());
					Vector<IndexValue> transferData = new Vector<IndexValue>();
					int dataSize = treeNode.getContent().getData().size();
					for (i = dataSize - 1; i > dataSize - 1 - numOfData; i--) {
						transferData.add(0, treeNode.getContent().getData()
								.get(i));
						treeNode.getContent().getData().remove(i);
					}
					...
					if (maxValue.getType() == IndexValue.NUMERIC_TYPE)
						newNodeMaxValue = new BoundaryValue(oldMaxValue
								.getString(), Long.parseLong(maxValue
								.getString()));
					else
						newNodeMaxValue = new BoundaryValue(maxValue
								.getString(), oldMaxValue.getLongValue());

					treeNode.getContent().setMaxValue(newNodeMaxValue);

					Body body = new SPLBSplitDataBody(serverpeer
							.getPhysicalInfo(), treeNode.getLogicalInfo(),
							false, treeNode.getContent().getMaxValue(),
							oldMaxValue, transferData, treeNode
