...
public class SPLeaveListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if (treeNode.getContent().getMaxValue().compareTo(
					body.getContent().getMinValue()) == 0) {
				treeNode.getContent().setMaxValue(
						body.getContent().getMaxValue());
			} else if (treeNode.getContent().getMinValue().compareTo(
					body.getContent().getMaxValue()) == 0) {
				treeNode.getContent().setMinValue(
						body.getContent().getMinValue());
			} else {
				if (body.getContent().getMinValue().compareTo(
						treeNode.getContent().getMinValue()) < 0) {
					treeNode.getContent().setMinValue(
							body.getContent().getMinValue());
				}
				if (body.getContent().getMaxValue().compareTo(
						treeNode.getContent().getMaxValue()) > 0) {
					treeNode.getContent().setMaxValue(
							body.getContent().getMaxValue());
				}
			}
			...
			if (!direction) {
				...
				if (treeNode.getLogicalInfo().getNumber() == 1)
					treeNode.getContent().setMinValue(
							new BoundaryValue(IndexValue.MIN_KEY.getString(),
									Long.MIN_VALUE));
			} else {
				treeNode.setRightAdjacentNode(adjacentInfo);
				treeNode.setRightChild(null);
				if (treeNode.getLogicalInfo().getNumber() == PeerMath.pow(2,
			...
			e.printStackTrace();
			throw new EventHandleException("Super peer leaves network failure",
					e);
		}
	}
