...
public class SPJoinAcceptListener extends ActionAdapter {
	...
	public void actionPerformed(PhysicalInfo dest, Message msg)
		...
		try {
			...
			if (body.getLogicalSender() == null)
				parentNode = null;
			else
				parentNode = new ParentNodeInfo(body.getPhysicalSender(), body
						.getLogicalSender());
			nodeNumber = body.getNewNodePosition().getNumber();
			nodeLevel = body.getNewNodePosition().getLevel();
			leftRTSize = PeerMath.getRoutingTableSize(nodeNumber, nodeLevel,
					false);
			rightRTSize = PeerMath.getRoutingTableSize(nodeNumber, nodeLevel,
					true);
			if (body.getNumOfExpectedRTReply() == 0)
				status = 1;

			SPGeneralAction.saveData(body.getContent().getData());

			// create new node and add to the existing list node
			TreeNode treeNode = new TreeNode(body.getNewNodePosition(),
					parentNode, null, null, body.getLeftAdjacent(), body
							.getRightAdjacent(), new RoutingTableInfo(
							leftRTSize), new RoutingTableInfo(rightRTSize),
					body.getContent(), body.getNumOfExpectedRTReply(), status);
			...
			if (!body.getIsFake()) {
				...
				// boolean result =
				SPGeneralAction.transferFakeNode(serverpeer, treeNode, body
						.getDirection(), true, body.getPhysicalSender());
				// ActivateActiveStatus activateActiveStatus =
				new ActivateActiveStatus(treeNode,
						ServerPeer.TIME_TO_STABLE_STATE);
			}

			// examine the indexed data, and republish it, if they are not in
			// the range
			...
			try {
				...
				while (rs.next()) {
					...
					if (key.compareTo(treeNode.getContent().getMinValue()
							.getStringValue()) >= 0
							|| key.compareTo(treeNode.getContent()
									.getMaxValue().getStringValue()) < 0)
						continue;
					...
					for (int i = 0; i < ips.length; i++) {
						...
						if (tables.containsKey(next)) {
							Vector<LocalTableIndex> nextTables = tables
									.get(next);
			...
			try {
				...
				while (rs.next()) {
					...
					for (int i = 0; i < ips.length; i++) {
						...
						if (columns.containsKey(next)) {
							...
							if (!flag) {
								LocalColumnIndex cindex = new LocalColumnIndex(
							...
							LocalColumnIndex cindex = new LocalColumnIndex(key,
									new Vector<String>());
							cindex.getListOfTables().add(data[2]);
							nextColumn.add(cindex);
				...
				while (enu.hasMoreElements()) {
					...
					ERPInsertColumnIndexBody mybody = new ERPInsertColumnIndexBody(
							serverpeer.getPhysicalInfo(), next, columns
									.get(next), treeNode.getLogicalInfo());
					Head head = new Head();
			...
			try {
				...
				while (rs.next()) {
					...
					for (int i = 0; i < ips.length; i++) {
						...
						if (tuples.containsKey(next)) {
							Vector<LocalDataIndex> nextColumn = tuples
									.get(next);
							LocalDataIndex cindex = new LocalDataIndex(key,
									data[2]);
							nextColumn.add(cindex);
						} else {
				...
				while (enu.hasMoreElements()) {
					PhysicalInfo next = (PhysicalInfo) enu.nextElement();
					ERPInsertDataIndexBody mybody = new ERPInsertDataIndexBody(
							serverpeer.getPhysicalInfo(), next, tuples
									.get(next),
							ERPInsertDataIndexBody.STRING_TYPE, treeNode
									.getLogicalInfo());
					Head head = new Head();
			...
			throw new EventHandleException(
					"Super peer joins network but cannot be accepted", e);
		}
	}
