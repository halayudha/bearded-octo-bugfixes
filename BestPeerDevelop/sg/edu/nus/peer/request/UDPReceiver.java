...
/**
 * The <code>UDPReceiver</code> is responsible for listening and receiving UDP
 * packets from remote sites by wrapping <code>DatagramSocket</code>.
 * 
 * <p>
 * All concrete receivers should extend the <code>UDPReceiver</code> with a set
 * of <code>UDPHandler</code>, each of which is responsible for processing the
 * received UDP packets.
 * 
 * <p>
 * To handle UDP packets, a callback pattern is used by <code>UDPHandler</code>.
 * To pass <code>UDPListener</code>s to <code>UDPHandler</code>, the
 * <code>UDPReceiver</code> provides the methods
 * <code>{@link #addUDPListener(UDPListener)}</code> and
 * <code>{@link #removeUDPListener(UDPListener)}</code> to register and remove a
 * <code>UDPListener</code> to its <code>UDPHandler</code>s.
 * 
 * @author Xu Linhao
...
public abstract class UDPReceiver implements Runnable {
	...
	/**
	 * The signal used to determine whether stops the service of accepting the
	...
	/**
	 * Construct the <code>UDPReceiver</code> with specified
	 * <code>DatagramSocket</code> and the maximum number of UDP packets that
	 * can be handled simultaneously.
	 * 
	 * @param gui
	 *            the main frame
	 * @param ds
	 *            the instance of a <code>DatagramSocket</code>
	 * @param maxConn
	 *            the maximum number of UDP packets that can be handled
	 *            simultaneously
	 * @throws SocketException
	 */
	public UDPReceiver(AbstractMainFrame gui, DatagramSocket ds, int maxConn)
			throws SocketException {
		this.gui = gui;
	 ...
	 * 
	 * @param l
	 *            the <code>UDPListener</code> that is responsible for
	...
	/**
	 * Initiate a set of <code>UDPHandler</code>s and the maximum number of
	 * handlers is specified by the parameter <code>maxConn</code>.
	 */
	public abstract void setupHandlers();

	/**
	 * Listening UDP packets from remote servers and if any UDP packet is
	 * received, then the packet will be passed to the handlers in order to
	 * process it.
	 */
	protected void receive() {
		...
		while (!stop) {
			try {
				DatagramPacket incoming = new DatagramPacket(buffer, 0,
						buffer.length);
				...
				if (!stop) // if stop flag is set as true, just ignore exception
				{
					LogManager.LogException(
							"Exception ocurrs in receive of UDPReceiver", e);
				}
			}
		}
	}
	...
	/**
	 * Accept an incoming UDP packet and then pass it to the
	 * <code>UDPHandler</code>s for processing.
	 * 
	 * @param request
	...
	/**
	 * Setup all <code>UDPHandler</code>s and then be prepared for receiving the
	 * incoming UDP packets.
	 */
	public void run() {
		this.receive();
