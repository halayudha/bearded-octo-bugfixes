...
public class ServerPnPServer extends UDPServer {
	 ...
	 */
	public ServerPnPServer(AbstractMainFrame gui, int port, int maxConn,
			long period) throws SocketException {
	...
	public void run() {
		try {
			...
			// init receiver
			receiver = new ServerPnPReceiver(gui, sender.getDatagramSocket(),
					maxConn);
			receiver.addUDPListener((ServerPnPSender) sender);
			receiverThread = new Thread(receiver,
					"Pong Receiver of Server Peer");
