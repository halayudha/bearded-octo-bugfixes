...
public class PhysicalInfo implements Serializable {
	...
	public PhysicalInfo(String serializeData) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Incorrect serialize data at PhysicalInfo:"
					+ serializeData);
			/*
	 ...
	 * @param serializeData the serialized string value
	 */
	public PhysicalInfo(String serializeData, String delim) {
		String[] arrData = serializeData.split(delim);
		try {
			this.ip = arrData[0];
			this.port = Integer.parseInt(arrData[1]);
		} catch (Exception e) {
