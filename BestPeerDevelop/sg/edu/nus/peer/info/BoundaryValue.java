...
public class BoundaryValue implements Comparable, Serializable {
	...
	public BoundaryValue(String serializeData) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Incorrect serialize data at BoundaryValue:"
					+ serializeData);
		}
	}
	...
	public static BoundaryValue average(BoundaryValue value1,
		...
		long midLongValue = (long) ((value1.getLongValue() + value2
				.getLongValue()) / 2);
	...
	public int compareTo(IndexValue compareValue) throws ClassCastException {
		if (compareValue.getType() == IndexValue.NUMERIC_TYPE)
			return (Long.valueOf(this.longValue)).compareTo(Long
					.valueOf(compareValue.getString()));
	...
	public int compareTo(Object compareObject) throws ClassCastException {
		if (compareObject instanceof BoundaryValue) {
			...
			if (this.longValue != compareValue.longValue)
				return (Long.valueOf(this.longValue)).compareTo(Long
						.valueOf(compareValue.longValue));
			else
				return (this.stringValue).compareTo(compareValue.stringValue);
		}
		// VHTam
		else if (compareObject instanceof String) {
			return (this.stringValue).compareTo((String) compareObject);
		}
		// end VHTam
