...
public class IndexValue implements Comparable, Serializable {
	...
	public final static IndexValue MIN_KEY = new IndexValue(STRING_TYPE, String
			.valueOf(MIN_CHARACTER));

	/**
	 * The maximum character value and the maximum index key.
	 */
	// private final static char MAX_CHARACTER = 'z';
	public final static IndexValue MAX_KEY = new IndexValue(STRING_TYPE, String
			.valueOf("zzzzzzzzzzzzzzzzzzzz"));
	...
	public IndexValue(String serializeData) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Incorrect serialize data for IndexValue:"
					+ serializeData);
		}
	}
		...
		if (compareObject instanceof IndexValue) {
			...
			case NUMERIC_TYPE:
				return ((Integer) this.value)
						.compareTo((Integer) compareValue.value);

			case STRING_TYPE:
				return ((String) this.value)
						.compareTo((String) compareValue.value);
			...
			case RANGE_INDEX_TYPE:
				...
				if (range1.getType() == Schema.INTEGER_TYPE) {
					return (new Integer(range2.getMinValue()))
							.compareTo(new Integer(range1.getMinValue()));
				} else
					return range2.getMinValue().compareTo(range1.getMinValue());
			default:
			...
			return ((String) this.value).compareTo((String) compareValue
					.getStringValue());
		}
		throw new ClassCastException("Object is invalid");
	}
	 ...
	 * @throws ClassCastException
	 */
	public static IndexValue average(IndexValue value1, IndexValue value2)
			throws ClassCastException {
		if ((value1.type == NUMERIC_TYPE) && (value2.type == NUMERIC_TYPE)) {
			int result = (((Integer) value1.value).intValue() + ((Integer) value2.value)
					.intValue()) / 2;
				...
				if (c1 != c2) {
					...
					if (PeerMath.isLowerCase(c1)) {
						if (!PeerMath.isLowerCase(c2)) {
							 ...
							 */
							while (!PeerMath.isLowerCase((char) mid)
									&& (mid > c1)) {
								mid--;
							}
						}
					...
					/* if the minimal value is upper case */
						...
						if (PeerMath.isUpperCase(c2)) {
							...
							while (!PeerMath.isUpperCase((char) mid)
									&& (mid > c1)) {
								mid--;
							}
						} else {
