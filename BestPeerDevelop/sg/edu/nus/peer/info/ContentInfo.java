...
public class ContentInfo implements Serializable {
	 ...
	 */
	public ContentInfo(BoundaryValue minValue, BoundaryValue maxValue,
			int order, Vector<IndexValue> data) {
	...
	public ContentInfo(String serializeData) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Incorrect serialize data at ContentInfo:"
					+ serializeData);
		}
	}
		...
		case INSERT_AS_MIN_KEY:
			...
			if (this.data.size() != 0) {
				...
				while (i < this.data.size()) {
					if (((IndexValue) this.data.get(i))
							.compareTo(sentinelValue) > 0) {
	...
	public String toString() {
		...
		if (this.data.size() == 0)
			...
			outMsg += ((IndexValue) this.data.get(this.data.size() - 1))
					.toString();
		}
		return outMsg;
	}
	...
	public String serialize() {
		String outMsg;
		outMsg = "MinValue=" + this.minValue + ",MaxValue=" + this.maxValue
				+ ",Order=" + this.order;
		if (this.data.size() == 0)
			outMsg += ",Data=null";
		else {
