...
import sg.edu.nus.gui.AbstractConfigDialog;
...
public class ConfigDialog extends AbstractConfigDialog {
	...
	public ConfigDialog(BootstrapGUI gui) {
		super(gui, LanguageLoader.getProperty("label.conf"), true, 300, 200, ""
				+ Bootstrap.LOCAL_SERVER_PORT, "" + Bootstrap.CAPACITY);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (cmd.equals(command[0])) {
			if (checkValue()) {
				...
				Bootstrap.LOCAL_SERVER_PORT = Integer.parseInt(tfPort.getText()
						.trim());
				Bootstrap.CAPACITY = Integer.parseInt(tfCapacity.getText()
						.trim());
				Bootstrap.write();
