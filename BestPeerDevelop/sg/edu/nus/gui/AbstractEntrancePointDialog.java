...
public abstract class AbstractEntrancePointDialog extends AbstractDialog
	 ...
	 * The name of buttons.
	 */
	String join = LanguageLoader.getProperty("button.join");
	String cancel = LanguageLoader.getProperty("button.cancel");
	 ...
	 */
	public AbstractEntrancePointDialog(Frame gui, String title, boolean model,
			int height, int width, PeerInfo[] data) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			entranceTableModel.insertRow(rowIdx, data[i]
	...
	protected JPanel makeContentPane(PeerInfo[] data) {
		String peerID = LanguageLoader.getProperty("label.peerID");
		String IpAddress = LanguageLoader.getProperty("label.ipAddress");
		String port = LanguageLoader.getProperty("label.port");

		JPanel panel = new JPanel(new BorderLayout());
		// String[] columns = { "Peer ID", "IP Address", "Port" };
		String[] columns = { peerID, IpAddress, port };
		...
		panel.setBorder(BorderFactory.createTitledBorder(null,
				"Select An Entrance Point", TitledBorder.DEFAULT_JUSTIFICATION,
