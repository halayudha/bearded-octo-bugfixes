...
public class CWTabbedPaneUI extends BasicTabbedPaneUI {
	...
	protected void paintTabArea(Graphics g, int tabPlacement, int selectedIndex) {
		...
		// VHTam
		// very good
		// fill gradient theme background first
		int th = tabPane.getBounds().height;
		Graphics2D g2d = (Graphics2D) g;
		Color startColor = GuiLoader.panelStartColor;
		Color endColor = GuiLoader.panelEndColor;
		GradientPaint GP = new GradientPaint(0, 0, startColor, tw, 0, endColor,
				true);
		g2d.setPaint(GP);
	...
	protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex,
			int tx, int ty, int tw, int th, boolean isSelected) {
		Graphics2D g2d = (Graphics2D) g;
		...
		if (isSelected) {
			...
			g.fillRect(buttonHeight / 2, buttonHeight, tw - buttonHeight,
					buttonHeight / 2 + 1);
			...
			g.drawLine(buttonHeight / 2, buttonHeight + (buttonHeight / 2) + 1,
					tw - buttonHeight / 2, buttonHeight + (buttonHeight / 2)
							+ 1);
			g.drawLine(tw - buttonHeight / 2,
					buttonHeight + (buttonHeight / 2), tw, buttonHeight);
			...
			for (int i = 1; i <= fadeColorCount; i++) {
				...
				g.drawLine(buttonHeight / 2 + 2 + i, buttonHeight
						+ (buttonHeight / 2) + 2 + i,
						tw - buttonHeight / 2 - 1, buttonHeight
								+ (buttonHeight / 2) + 2 + i);
				g.drawLine(tw - buttonHeight / 2 - 1, buttonHeight
						+ (buttonHeight / 2) + 2 + i, tw, buttonHeight + 1 + i);
			}
		} else {
			g.setColor(tabColor);
			...
			for (int i = 1; i <= fadeColorCount; i++) {
				g.setColor(fadeColors[fadeColorCount - 1]);
				g.drawLine(0, buttonHeight + 1 + i, tw - 1, buttonHeight + 1
						+ i);
			}
		}
	...
	protected void paintText(Graphics g, int tabPlacement, Font font,
		...
		if (isSelected) {
			...
			g.drawString(title, (r.width / 2 - fm.stringWidth(title) / 2) + 1,
					buttonHeight / 2 + fm.getMaxDescent() + buttonHeight / 2
							+ 3);
