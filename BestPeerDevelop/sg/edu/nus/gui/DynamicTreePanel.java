...
public class DynamicTreePanel extends JPanel {
	...
	public DefaultMutableTreeNode getSelectedNode() {
		...
		if (currentSelection != null) {
			return (DefaultMutableTreeNode) (currentSelection
	...
	@SuppressWarnings("unchecked")
	private void findNodesAtLevel(TreeModel model, Object o, int levelToFind,
			int levelTraversing, Vector resultNodes) {
		int cc;
		...
		for (int i = 0; i < cc; i++) {
			...
			if (!model.isLeaf(child)) {
				findNodesAtLevel(model, child, levelToFind,
						levelTraversing + 1, resultNodes);
			}
		}
	}
	...
	private DefaultMutableTreeNode findParent(TreeModel model, Object o,
			String childNameToFind) {
		int cc;
		cc = model.getChildCount(o);
		for (int i = 0; i < cc; i++) {
			Object child = model.getChild(o, i);
	...
	private static int findNodeLevel(TreeModel model, Object o,
			DefaultMutableTreeNode node, int level) {
		if (o == node) {
			return level;
		}
