...
public class DBTree extends JTree {
		...
		while (tables.next()) {
			if (sharedInfoFile.exists()) {
				...
				try {
					...
					while (columns.next()) {
						columnNode = new DBTreeNode(columns
						...
						int columnStatus = checkColumnStatus(reader, tableName,
						...
						if (columnStatus == 1)
							columnNode
									.setShareStatus(DBTreeNode.SHARESTATUS_SHARE);
				...
				while (columns.next()) {
					...
					shareInfo += "COLUMN" + "|" + tableName + "|"
							+ columns.getString("COLUMN_NAME") + "|" + 0
							+ "\r\n";
				}
	...
	private int checkColumnStatus(BufferedReader reader, String tableName,
			String columnName) {
		int shared = -1;
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				String[] str = line.split("\\|");
				if ((str[0].equals("COLUMN")) && (str[1].equals(tableName))
						&& (str[2].equals(columnName))) {
	...
	final class DBTreePopupMenu extends JPopupMenu implements ActionListener {
		...
		private String[] itemName = { "Open Table", "View Schema", "Search...",
				"Share", "Unshare", "Reshare", "Refresh", "SchemaMapping" };

		private String[] commands = { "open", "schema", "search", "share",
				"unshare", "reshare", "refresh", "SchemaMapping" };
		...
		private void makeMenu() {
			...
			node = (DefaultMutableTreeNode) DBTree.this.getSelectionPath()
			...
			schemaMappingMenuItem = this.makeMenuItem(itemName[7], commands[7],
					images[7]);
			schemaMappingMenuItem.setBorder(BorderFactory.createEmptyBorder(2,
					offset, 2, 0));
			...
			// Added by Han Xixian, 2008-6-3
				...
				if (dbNode.getNodeType().equals("TABLE")) {
					...
					for (int i = 0; i < childCount; i++) {
						...
						// if(cNode.getShareStatus() ==
						// DBTreeNode.SHARESTATUS_TEMPUNSHARE)
						// break;
