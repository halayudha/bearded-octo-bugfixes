...
//import sg.edu.nus.search.event.IndexInsertBody;
/**
...
public class DBTableModel extends AbstractTableModel {
	...
	public Boolean[] shared; // boolean value indicates whether the data are
	 ...
	 */
	@SuppressWarnings("unchecked")
	DBTableModel(JDialog gui, ResultSet rs, boolean isShowTable)
			throws SQLException {
		...
		// If show the schema of table, retreival the column information from
		// the metadata of result set.
		else {
	...
	public int getColumnCount() {
		try {
			// If show the data in the table, return the number of columns in
			// the result set
			if (isShowTable) {
				return rsmd.getColumnCount() + 1;
			}
			// If show the schema of the table, return the number of information
			// user
	...
	public String getColumnName(int columnIndex) {
		try {
			// If show the data in the table, return the columnName in the
			// result set.
			if (isShowTable) {
				if (columnIndex == 0)
					return "shared?";
			...
			// If show the schema of the table, return the columnName of the
			// schema, like: "Field Name", "Data Type"
			else {
