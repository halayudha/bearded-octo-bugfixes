...
import sg.edu.nus.gui.sharing.DBConfigureDlg;
...
public class DBConfigureView extends JPanel {
	...
	private DBConfigureDlg parentComponent;
	 ...
	 */
	public DBConfigureView(DBConfigureDlg/*DatabaseExplorer*/parent) {
	...
	private void initPane() {
		...
		dbNameTextField = new JTextField(ServerPeer.SERVER_DB_NAME);

		JLabel usernameLabel = new JLabel("User Name");
		usernameTextField = new JTextField(ServerPeer.SERVER_DB_USER);

		JLabel pwLabel = new JLabel("Password:");
		pwField = new JTextField("Your password");
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					if (connectToDatabase(driver, url, port, dbName, username,
						...
						ServerPeer.SERVER_DB_DRIVER = driver;
						ServerPeer.SERVER_DB_URL = url;
						ServerPeer.SERVER_DB_USER = username;
						ServerPeer.SERVER_DB_PASS = password;
						ServerPeer.SERVER_DB_PORT = port;
						ServerPeer.SERVER_DB_NAME = dbName;

						DBConfigureView.this.parentComponent.dispose();
					}
				} catch (Exception e) {
					JOptionPane.showMessageDialog(
							DBConfigureView.this.parentComponent,
							"DBConfigure is unsuccessful\r\n, Check again.");
				}
			}
		});
		...
		connectButton = new JButton("Save");
	...
	private JComboBox listDrivers() {
		...
		cb.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				String item = DBConfigureView.this.driverComboBox
						.getSelectedItem().toString();
				if (item.equalsIgnoreCase("ODBC")) {
					// put the initial value when using ODBC driver
					// DBConnectView.this.urlTextField.setEnabled(false);
					// DBConnectView.this.portTextField.setEnabled(false);
				} else {
					DBConfigureView.this.urlTextField.setEnabled(true);
					DBConfigureView.this.portTextField.setEnabled(true);
	...
	public boolean connectToDatabase(String driver, String url, String port,
		...
		String urlFormat_globalSchema = null;
		String urlFormat_schemaMapping = null;
		...
		if (driver.equals("SQLSever")) {
			...
			urlFormat_globalSchema = "jdbc:mysql://" + url + "/"
					+ ServerPeer.GLOBAL_DB;
			urlFormat_schemaMapping = "jdbc:mysql://" + url + "/"
			...
			// System.err.println("Don't support such driver: " + driver);
		...
		ServerPeer.conn_localSchema = DriverManager.getConnection(urlFormat,
				username, pw); // get a connection
		ServerPeer.conn_globalSchema = DriverManager.getConnection(
				urlFormat_globalSchema, username, pw);
		ServerPeer.conn_schemaMapping = DriverManager.getConnection(
				urlFormat_schemaMapping, username, pw);

		// con = DriverManager.getConnection(DSN, USER, PWD); // get a
		// connection
		return true;
	}
