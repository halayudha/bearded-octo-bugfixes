...
public final class ConfigDialog extends AbstractConfigDialog {
	...
	public ConfigDialog(ServerGUI gui) {
		super(gui, "Configure Super Peer", true, 300, 200, ""
				+ ServerPeer.LOCAL_SERVER_PORT, "" + ServerPeer.CAPACITY);

		String title = LanguageLoader.getProperty("title.dlgConfigSuperPeer");
		this.setTitle(title);
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (cmd.equals(command[0])) {
			if (checkValue()) {
				...
				ServerPeer.LOCAL_SERVER_PORT = Integer.parseInt(tfPort
						.getText().trim());
				ServerPeer.CAPACITY = Integer.parseInt(tfCapacity.getText()
						.trim());
				ServerPeer.write();
				dispose();
			}
