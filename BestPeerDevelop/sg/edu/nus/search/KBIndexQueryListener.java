...
public class KBIndexQueryListener extends SPIndexOperateAdapter {
	 ...
	 */
	public KBIndexQueryListener(AbstractMainFrame gui, Analyzer analyzer) {
		super(gui, analyzer);
	...
	public void actionPerformed(IndexEvent event) throws IOException,
		...
		try {
			...
			if ((localParser != null) && (localParser.size() > 0)) {
				...
				if ((results != null) && (results.length > 0)) {
					serverPeer.sendMessage(requester, new Message(new Head(
							MsgType.KB_RESULT.getValue()), new SPResultBody(
							body.getQueryID(), results)));
				}
			}
			...
			if ((remoteParser != null) && (remoteParser.size() > 0)) {
				...
				Head head = new Head(MsgType.KB_QUERY.getValue());
	...
	public boolean isConsumed(IndexEvent event)
		...
		if (event.getHead().getValue() == IndexEventType.KB_SEARCH.getValue())
			return true;
		return false;
	}
