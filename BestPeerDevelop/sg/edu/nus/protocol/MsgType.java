...
public enum MsgType {
	...
	/**
	 * send global schema from bootstrap peer to peer
	 * 
	 * added by chensu
	 * 2009-4-29
	 */
	SCHEMA(common.key + 23),

	/**
	 * send role management information from bootstrap peer to peer
	 * 
	 * added by chensu
	 * 2009-4-29
	 */
	ROLE_MAN(common.key + 24),
	 ...
	 */
	SP_DELETE(superpeer_to_superpeer.key + 1), SP_DELETE_BUNDLE(
			superpeer_to_superpeer.key + 2),

	/**
	 * When a super peer inserts some data items.
	 */
	SP_INSERT(superpeer_to_superpeer.key + 3), SP_INSERT_BUNDLE(
			superpeer_to_superpeer.key + 4),
	 ...
	 */
	SP_LEAVE(superpeer_to_superpeer.key + 27), SP_LEAVE_URGENT(
			superpeer_to_superpeer.key + 28), SP_LEAVE_NOTIFY(
			superpeer_to_superpeer.key + 29), SP_LEAVE_NOTIFY_CLIENT(
			superpeer_to_superpeer.key + 30), SP_PASS_CLIENT(
			superpeer_to_superpeer.key + 31),
	 ...
	 */
	SP_SEARCH_EXACT(superpeer_to_superpeer.key + 36), SP_SEARCH_PAIR(
			superpeer_to_superpeer.key + 37), SP_SEARCH_EXACT_BUNDLE(
			superpeer_to_superpeer.key + 38),
	 ...
	 */
	SP_SEARCH_RANGE(superpeer_to_superpeer.key + 40), SP_SEARCH_RANGE_BUNDLE(
			superpeer_to_superpeer.key + 41),
	 ...
	 */
	SP_LI_ADJACENT(superpeer_to_superpeer.key + 49), SP_LI_ADJACENT_ROOT(
			superpeer_to_superpeer.key + 50),

	/**
	 * When a super peer replies the SP_LI_ADJACENT request
	 */
	SP_LI_ADJACENT_REPLY(superpeer_to_superpeer.key + 51), SP_LI_ADJACENT_ROOT_REPLY(
			superpeer_to_superpeer.key + 52),
	 ...
	 */
	SP_UPDATE(superpeer_to_superpeer.key + 58), SP_UPDATE_BUNDLE(
			superpeer_to_superpeer.key + 59),
	...
	// join query process
	query_processing(common.key + 3000), 
	TABLE_STAT_SEARCH(query_processing.key), 
	TABLE_STAT_RESULT(query_processing.key + 1), 
	JOIN_QUERY_PROCESSING(query_processing.key + 2),
	
	CACHE_DB_INDEX (query_processing.key+3),
	...
	index_search(common.key + 4000), TABLE_INDEX_SEARCH(index_search.key), TABLE_INDEX_SEARCH_RESULT(
			index_search.key + 1), QUERY_PEER(index_search.key + 2), QUERY_PEER_RESULT(
			index_search.key + 3), COLUMN_INDEX_SEARCH(index_search.key + 10),
	// end David adds

	// Wu Sai adds the message type for getting the query result
	query_result(common.key + 5000), TABLE_RETRIEVAL(query_result.key), TABLE_DATA(
			query_result.key + 1), WEB_SEARCH(query_result.key + 2), WEB_LOGIN(
			query_result.key + 3), WEB_LOGIN_ACK(query_result.key + 4),
	/**
	...
	public static String description(int msgType) {
		...
		} else if (WRONG_PASS.getValue() == msgType) {
		...
		// description of join query processing
		else if (MsgType.TABLE_STAT_SEARCH.getValue() == msgType) {
			result = "NORMAL PEER'S MESSAGE FOR RETRIEVING OTHERS' TABLE PARTITION STATISTICS";
		} else if (MsgType.TABLE_STAT_RESULT.getValue() == msgType) {
			result = "NORMAL PEER'S MESSAGE FOR RETURNING TABLE PARTITION STATISTICS";
		} else if (MsgType.JOIN_QUERY_PROCESSING.getValue() == msgType) {
			result = "JOIN_QUERY_PROCESSING";
		}

		else if (MsgType.CACHE_DB_INDEX.getValue() == msgType){
			result = "CACHE_DB_INDEX";
		}
		// end VHTam
		...
		} else if (MsgType.WEB_SEARCH.getValue() == msgType) {
			result = "QUERY REQUEST FROM WEB INTERFACE";
		} else if (MsgType.WEB_LOGIN.getValue() == msgType) {
			result = "LOGIN MESSAGE FROM THE WEB INTERFACE";
		} else if (MsgType.WEB_LOGIN_ACK.getValue() == msgType) {
			result = "LOGIN ACKNOWLEDGEMENT TO THE WEB INTERFACE";
		}

		// ---------- ERP INDEX -------------

		else if (MsgType.ERP_INSERT_TABLE_INDEX.getValue() == msgType) {
			result = "INSERT ERP TABLE INDEX";
			// added by chensu @ 2008-4-28
		} else if (MsgType.WRONG_PASS.getValue() == msgType) {
			result = "WRONG PASSWORD";
		} else if (MsgType.ROLE_MAN.getValue() == msgType) {
			result = "ROLE INFORMATION";
		} else if (MsgType.SCHEMA.getValue() == msgType) {
